...
const httpCreatedResponse = (data, reply) => {
  const status = (data && Object.keys(data)) ? 201 : 204
  return successResponse(data, null, status, reply)
}

const httpUpdatedResponse = (data, reply) => {
  const status = (data && Object.keys(data)) ? 200 : 204
  return successResponse(data, null, status, reply)
}
...
const httpFoundResponse = (data = [], metadata, reply) => {
  const status = (data && (data.length || Object.keys(data).length || data.toString().length))
    ? 200
    : 204
